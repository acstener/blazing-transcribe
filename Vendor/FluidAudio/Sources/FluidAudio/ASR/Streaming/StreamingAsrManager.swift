...
public actor StreamingAsrManager {
    ...
    private var lastConfirmedEmissionSample: Int = 0
    ...
    /// Stream already-resampled 16kHz mono PCM samples directly into the streaming pipeline.
    /// This avoids an extra conversion step when the caller already has realtime PCM blocks.
    public func streamSamples(_ samples: [Float]) async {
    ...
    public func finish() async throws -> String {
        ...
        let finalText = finalizeTranscriptText()

        logger.info("Final transcription: \(finalText.count) characters")
        return finalText
    }

    /// Flush the current utterance and return the latest transcript without closing the stream.
    /// This is intended for always-on dictation where the caller manages utterance boundaries.
    public func finalizeCurrentUtterance() async throws -> String {
        await flushRemaining()
        let finalText = finalizeTranscriptText()
        logger.info("Current utterance finalized: \(finalText.count) characters")
        return finalText
    }
    ...
    private func appendSamplesAndProcess(_ samples: [Float]) async {
        ...
        // The center window stays at `chunkSamples`, but we advance by the faster
        // hypothesis cadence so callers can surface volatile updates every 160ms.
        let chunk = max(config.chunkSamples, config.hypothesisChunkSamples)
        let advance = max(1, min(config.hypothesisChunkSamples, chunk))
        ...
        while currentAbsEnd >= (nextWindowCenterStart + chunk + right) {
            let centerStartAbs = nextWindowCenterStart
            ...
            await processWindow(
                ...
                emissionSample: centerStartAbs + chunk
            )

            // Advance by hypothesis stride for faster partial emissions.
            nextWindowCenterStart += advance
    ...
    private func flushRemaining() async {
        ...
        while currentAbsEnd > nextWindowCenterStart {  // process until we exhaust
            ...
            await processWindow(
                window,
                windowStartSample: leftStartAbs,
                emissionSample: nextWindowCenterStart + effectiveChunk,
    ...
    private func processWindow(
        ...
        do {
            ...
            let canPromoteConfirmed =
                isLastChunk ||
                shouldConfirm && (
                    lastConfirmedEmissionSample == 0 ||
                    (emissionSample - lastConfirmedEmissionSample) >= config.chunkSamples
                )
            ...
            await updateTranscriptionState(
                with: displayResult,
                shouldConfirm: shouldConfirm && canPromoteConfirmed,
                emissionSample: emissionSample
            )

            let liveTranscript = latestTranscriptText()
            guard !liveTranscript.isEmpty else { return }

            let update = StreamingTranscriptionUpdate(
                text: liveTranscript,
                isConfirmed: shouldConfirm && canPromoteConfirmed,
    ...
    private func updateTranscriptionState(
        ...
        let totalAudioProcessed = Double(bufferStartIndex + sampleBuffer.count) / 16000.0
        let incomingText = sanitizeTranscriptText(result.text)
        guard !incomingText.isEmpty else { return }

        let mergedTranscript = mergeTranscript(base: confirmedTranscript, incoming: incomingText)
        let didAdvance = mergedTranscript != confirmedTranscript

        if shouldConfirm {
            if didAdvance {
                lastConfirmedEmissionSample = emissionSample
            }
            confirmedTranscript = mergedTranscript
            volatileTranscript = mergedTranscript
            logger.debug(
                "CONFIRMED (\(result.confidence), \(String(format: "%.1f", totalAudioProcessed))s context): merged '\(mergedTranscript)' (advanced=\(didAdvance))"
            )
        } else {
            volatileTranscript = mergedTranscript
            let hasMinimumContext = totalAudioProcessed >= config.minContextForConfirmation
            ...
            logger.debug("VOLATILE (\(result.confidence)): \(reason) - merged '\(mergedTranscript)'")
        }
    }

    private func finalizeTranscriptText() -> String {
        let stateDrivenText = latestTranscriptText()
        if !stateDrivenText.isEmpty {
            return stateDrivenText
        }
    ...
    private func latestTranscriptText() -> String {
        let volatile = sanitizeTranscriptText(volatileTranscript)
        if !volatile.isEmpty { return volatile }
        return sanitizeTranscriptText(confirmedTranscript)
    }
    ...
    private func mergeTranscript(base: String, incoming: String) -> String {
        let normalizedBase = sanitizeTranscriptText(base)
        let normalizedIncoming = sanitizeTranscriptText(incoming)

        guard !normalizedIncoming.isEmpty else { return normalizedBase }
        guard !normalizedBase.isEmpty else { return normalizedIncoming }

        if normalizedIncoming == normalizedBase {
            return normalizedBase
        }
        if normalizedIncoming.hasPrefix(normalizedBase) {
            return normalizedIncoming
        }
        if normalizedBase.hasPrefix(normalizedIncoming) {
            return normalizedBase
        }
        if normalizedBase.hasSuffix(normalizedIncoming) {
            return normalizedBase
        }

        let minLength = min(normalizedBase.count, normalizedIncoming.count)
        if minLength >= 8 {
            let sharedPrefix = sharedPrefixCharacterCount(normalizedBase, normalizedIncoming)
            let prefixRatio = Double(sharedPrefix) / Double(minLength)
            if prefixRatio >= 0.7 {
                // Prefer corrected full hypotheses over appending near-duplicate tails.
                return normalizedIncoming
            }
        }

        let baseWords = normalizedBase.split(separator: " ")
        let incomingWords = normalizedIncoming.split(separator: " ")
        let maxOverlap = min(baseWords.count, incomingWords.count)

        if maxOverlap > 0 {
            for overlap in stride(from: maxOverlap, through: 1, by: -1) {
                let baseSuffix = baseWords.suffix(overlap)
                let incomingPrefix = incomingWords.prefix(overlap)
                if Array(baseSuffix) == Array(incomingPrefix) {
                    let appended = incomingWords.dropFirst(overlap)
                    if appended.isEmpty {
                        return normalizedBase
                    }
    ...
    private func sharedPrefixCharacterCount(_ lhs: String, _ rhs: String) -> Int {
        var count = 0
